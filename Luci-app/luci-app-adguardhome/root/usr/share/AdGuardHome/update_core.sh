...
resolve_download_arch(){
	manual_arch="$(uci -q get AdGuardHome.AdGuardHome.downloadarch 2>/dev/null)"
	case "$manual_arch" in
	""|"auto")
		auto_detect_download_arch
		;;
	...
	"powerpc64")
		...
		;;
	*)
		echo -e "unknown manual download arch $manual_arch, fallback to auto detect"
		auto_detect_download_arch
		;;
	esac

	[ "$manual_arch" = "" ] || [ "$manual_arch" = "auto" ] || echo -e "manual download arch override: $manual_arch -> $Arch"
}
...
check_latest_version(){
	...
	if [ "$release_channel" = "beta" ]; then
		...
		echo -e "Beta channel selected. Local version: ${now_ver}. Downloading latest beta build."
		doupdate_core
	fi
